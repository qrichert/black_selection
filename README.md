...
```shell
python3 black_selection.py /path/to/black file.py start_line end_line [args]

# Note that `args` will be forwarded to black. Example with line length of 79:
python3 black_selection.py /path/to/black file.py start_line end_line -l 79
...
---------------------
 ...
 - Fill in:
   ...
   - Arguments: `/path/to/black $FilePath$ $SelectionStartLine$ $SelectionEndLine$`  
                (Add `-l 79` at the end for strict PEP8, default line length is 88.)
...
To add a shortcut:
 ...
 - File > Settings > Keymap > External tools > Search for "black (selection)"
 - Add shortcut: (e.g.) AltGr + B
 - OK
 - Select lines > AltGr + B
